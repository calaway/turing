...
class ReduceTest < Minitest::Test
  ...
  def test_subtract_list_of_numbers
    ...
    result = numbers.reduce(0) do |difference, number|
       difference - number# Your code goes here
    end
  ...
  def test_multiply_list_of_numbers
    numbers = [2, 3, 5, 7]
    result = numbers.inject(1) do |product, num|
      num * product
    end# initial value is 1
    # Your code goes here
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    keywords = ["fish", "blue"]
    result = keywords.inject('one fish two fish red fish blue fish') do |phrase, keyword|
  ...
  def test_divide_560_by_a_bunch_of_numbers
    numbers = [2, 2, 2, 5, 7]
    result = numbers.inject(560) do |quotient, num|
      quotient / num
    end
    # Your code goes here
